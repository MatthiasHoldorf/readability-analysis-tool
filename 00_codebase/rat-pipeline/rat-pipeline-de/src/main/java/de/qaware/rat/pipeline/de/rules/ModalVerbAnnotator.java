...
/**
 * This {@code Java Annotator} class detects modal verbs in the text.
 * 
 * @author Matthias
 *
 */
...
public class ModalVerbAnnotator extends JCasAnnotator_ImplBase {
    private static final Logger LOGGER = LoggerFactory.getLogger(ModalVerbAnnotator.class);
    ...
    public void process(JCas aJCas) {
        LOGGER.info("Start ModalVerbAnnotator");
        ...
        try {
            ...
            for (Token word : words) {
                if (Arrays.asList(modalVerbs).contains(word.getCoveredText().toLowerCase())) {
                    UimaUtils.createRatReadabilityAnomaly(aJCas, "ModalVerb", "ReadabilityAnomaly", severity,
                            "Vermeiden Sie Modalitäten.", 1, word.getBegin(), word.getEnd());
                }
            }

        } catch (IOException e) {
            LOGGER.error("The ModalVerbAnnotator failed.");
        }
    }
}